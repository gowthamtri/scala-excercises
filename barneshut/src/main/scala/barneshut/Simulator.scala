...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val combined = new Boundaries()
    combined.minX = Math.min(a.minX, b.minX)
    combined.minY = Math.min(a.minY, b.minY)
    combined.maxX = Math.max(a.maxX, b.maxX)
    combined.maxY = Math.max(a.maxY, b.maxY)
    combined
  }
