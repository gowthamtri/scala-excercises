...
object LineOfSight extends LineOfSightInterface {
  ...
  def getMax(a: Float, b: Float): Float = if (a > b) a else b

  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var i = 1
    output(0) = 0f
    while (i < input.length) {
      output(i) = getMax(output(i - 1), input(i) / i)
      i = i + 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    var res: Float = 0f
    while (i < until) {
      res = if(i == 0) 0 else getMax(res, input(i) / i)
      i = i + 1
    }
    res
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    tree match {
      ...
      case Node(left, right) => parallel(
        ...
        downsweep(input, output, getMax(startingAngle, left.maxPrevious), right))
    }
  }
