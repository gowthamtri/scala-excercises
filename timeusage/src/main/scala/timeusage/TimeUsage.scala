...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row =
    Row.fromSeq(line.head.replaceAll("\"", "") :: line.tail.map(_.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    (
      columnNames.filter(n => n.startsWith("t01") || n.startsWith("t03") || n.startsWith("t11") || n.startsWith("t1801")
        || n.startsWith("t1803")).map(new Column(_)),
      columnNames.filter(n => n.startsWith("t05") || n.startsWith("t1805")).map(new Column(_)),
      columnNames.filter(n => n.startsWith("t02") || n.startsWith("t04") || n.startsWith("t06") || n.startsWith("t07")
        || n.startsWith("t08") || n.startsWith("t09") || n.startsWith("t10") || n.startsWith("t12") || n.startsWith("t13")
        || n.startsWith("t14") || n.startsWith("t15") || n.startsWith("t16")
        || (n.startsWith("t18") && !n.startsWith("t1801") && !n.startsWith("t1803") && !n.startsWith("t1805"))).map(new Column(_))
    )
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce(_ + _).divide(60f).as("primaryNeeds")
    val workProjection: Column = workColumns.reduce(_ + _).divide(60f).as("work")
    val otherProjection: Column = otherColumns.reduce(_ + _).divide(60f).as("other")
