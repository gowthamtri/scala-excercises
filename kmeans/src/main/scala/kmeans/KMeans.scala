...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    if (points.isEmpty) means.map(m => m -> Seq()).toMap else points.groupBy(point => findClosest(point, means))
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    if (points.isEmpty) means.map(m => m -> ParSeq()).toMap else points.groupBy(point => findClosest(point, means))
  }
