...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction {
      case Json.Num(n) if n.isValidInt => n.bigDecimal.intValue()
    }
  ...
  implicit val booleanDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(s) => s}
  ...
  def getFirst[A](x: Option[A]): A = x match {
    case Some(s) => s
  }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] = {
    /*Decoder.fromPartialFunction {
      case Json.Arr(as) => getFirst(unfold(as.map(x => decoder.decode(x))))
    }*/

    Decoder.fromPartialFunction {
      case Json.Arr(as) => as.map(decoder.decode).map(x => getFirst(x))
    }
  }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] = {
    Decoder.fromFunction{
      case Json.Obj(obj) => decoder.decode(getFirst(obj.get(name)))
    }
  }
